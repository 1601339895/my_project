...
class IG_MSA(nn.Module):
    ...
    def forward(self, x_in, illu_fea_trans):
        ...
        q_inp = self.to_q(x)  #torch.Size([1, 65536, 48])
        k_inp = self.to_k(x)
        v_inp = self.to_v(x)
        illu_attn = illu_fea_trans # torch.Size([1, 65536, 48])
        ...
        x = attn @ v   # b,heads,d,hw   [b,h,d,hw]
        x = x.permute(0, 3, 1, 2)    # Transpose  [b,hw,h,d]
        x = x.reshape(b, h * w, self.num_heads * self.dim_head) #[b,hw,c]
...
class RetinexFormer(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=48, stage=1, num_blocks=[1,2,2]):
