...
from demo_test import New_AdapterLayer
...
class FFTAttention(nn.Module):
    def __init__(self, dim: int, **kwargs):
        ...
        self.patch_size = kwargs["patch_size"]  # 设置patch_size

        self.q = nn.Conv2d(dim, dim, kernel_size=1, bias=False)  # 用于生成查询（query）矩阵。该卷积层的输入和输出维度都为 dim，卷积核大小为 1x1
        self.q_dwconv = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, groups=dim)  # 用于生成查询（query）矩阵的扩充卷积层
        self.kv = nn.Conv2d(dim, dim * 2, kernel_size=1,
                            bias=False)  # 用于生成键值（key-value）矩阵。该卷积层的输入维度为 dim，输出维度为 dim*2，卷积核大小为 1x1
        self.kv_dwconv = nn.Conv2d(dim * 2, dim * 2, kernel_size=7, stride=1, padding=7 // 2,
                                   groups=dim * 2)  # 用于生成键值（key-value）矩阵的扩充卷积层
        self.norm = LayerNorm(dim, "WithBias")  # 用于层归一化特征图
        self.proj_out = nn.Conv2d(dim, dim, kernel_size=1, padding=0)  # 用于生成输出特征图
    ...
    def pad_and_rearrange(self, x):
        ...
        pad_h = (self.patch_size - (h % self.patch_size)) % self.patch_size  # 计算需要填充的高度
        pad_w = (self.patch_size - (w % self.patch_size)) % self.patch_size  # 计算需要填充的宽度
        x = F.pad(x, (0, pad_w, 0, pad_h), mode='constant', value=0)  # 填充特征图
        x = rearrange(x, 'b c (h p1) (w p2) -> b c h w p1 p2', p1=self.patch_size, p2=self.patch_size)  # 重排特征图
        return x  # b c h w p1 p2

    def rearrange_to_original(self, x, x_shape):
        h, w = x_shape # 获取原始特征图的尺寸
        x = rearrange(x, 'b c h w p1 p2 -> b c (h p1) (w p2)', p1=self.patch_size, p2=self.patch_size)  # 重排特征图
        x = x[:, :, :h, :w]  # Slice out the original height and width，使用切片的方式获取原始的高度和宽度
        return x  # b c h w

    def forward(self, x):
        b, c, h, w = x.shape    # 获取输入特征图的尺寸

        q = self.q_dwconv(self.q(x))  # 生成查询（query）矩阵
        kv = self.kv_dwconv(self.kv(x))  # 生成键值（key-value）矩阵
        k, v = kv.chunk(2, dim=1)  # 切分键值矩阵为 k 和 v

        q = self.pad_and_rearrange(q)  # 填充和重排查询（query）矩阵
        k = self.pad_and_rearrange(k)  # 填充和重排键值（key-value）矩阵

        q_fft = torch.fft.rfft2(q.float())  # 快速傅里叶变换
        k_fft = torch.fft.rfft2(k.float())  # 快速傅里叶变换
        out = q_fft * k_fft  # 点乘
        out = torch.fft.irfft2(out, s=(self.patch_size, self.patch_size))  # 逆快速傅里叶变换

        out = self.rearrange_to_original(out, (h, w))  # 重排输出特征图

        out = self.norm(out)  # 层归一化输出特征图
        out = out * v  # 点乘输出特征图与值矩阵

        out = self.proj_out(out)  # 生成输出特征图
        return out  # b c h w
...
class RoutingFunction(nn.Module):
    def __init__(self, dim, freq_dim, num_experts, k,
                 complexity, use_complexity_bias: bool = True,
