...
class MANet(nn.Module):
    def __init__(self, dim, bias):
        ...
        hidden_dim = 2 * dim
        self.first_conv = nn.Conv2d(dim, hidden_dim, kernel_size=1, bias=bias)

        self.second_conv = nn.Sequential(
            nn.Conv2d(hidden_dim, hidden_dim, kernel_size=1, bias=bias),
            # nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, padding=1, groups=hidden_dim, bias=bias),
            nn.Conv2d(dim, dim, 3, 1, 1, bias=bias,groups=hidden_dim),
            nn.Conv2d(hidden_dim, hidden_dim, kernel_size=1, bias=bias)
        )
...
class IG_MSA(nn.Module):
    ...
    def forward(self, x_in, illu_fea):
        ...
        illu_mod = self.illumination_modulator(illu_fea)
...
class DecoderBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, expert_layer, complexity_scale=None,
                 ...
                 freq_dim: int = 128, with_complexity: bool = False, DW_Expand=2, dilations=[1, 2, 4],
...
if __name__ == '__main__':
    model = IGMoCEIR(rank=2, num_blocks=[4, 6, 6, 8],
                     ...
                     expert_layer=FFTAttention,
                     DW_Expand=2,
                     dilations=[1, 4, 8, 16], fusion_weight=0.5, bias=True).cuda()
    # 计算模型参数量
