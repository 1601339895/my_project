...
from net.LimiSenseMoE import LumiSenseMoEIR
...
def run_test(opts, net, dataset, factor=8):
    ...
    with torch.no_grad():
        for ([clean_name, de_id], degrad_patch, clean_patch) in tqdm(testloader):
            # 检查输入数据是否有异常值
            print(f"Degraded patch min: {degrad_patch.min()}, max: {degrad_patch.max()}")
            print(f"Clean patch min: {clean_patch.min()}, max: {clean_patch.max()}")
            ...
            # 检查模型输出是否有异常值
            print(f"Restored output min: {restored.min()}, max: {restored.max()}")
            assert not torch.isnan(restored).any(), "Model output contains NaN"
            ...
            restored_cpu = restored.cpu().detach().permute(0, 2, 3, 1).squeeze(0).numpy()
            degrad_patch_cpu = degrad_patch.cpu().detach().permute(0, 2, 3, 1).squeeze(0).numpy()
            clean_cpu = clean_patch.cpu().detach().permute(0, 2, 3, 1).squeeze(0).numpy()

            # 确保所有输入都是有效的
            assert not np.any(np.isnan(restored_cpu)), "Restored image contains NaN"
            assert not np.any(np.isnan(clean_cpu)), "Clean image contains NaN"

            ssim_temp = calc_ssim(clean_cpu, restored_cpu)
            ssim.append(ssim_temp)
            psnr_temp = peak_signal_noise_ratio(clean_cpu, restored_cpu, data_range=1)
            psnr.append(psnr_temp)

            print(f"PSNR: {psnr_temp}, SSIM: {ssim_temp}, LPIPS: {lpips[-1]}")
