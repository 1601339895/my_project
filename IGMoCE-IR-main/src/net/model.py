...
class Mix(nn.Module):
    def __init__(self, dim, num_heads, LayerNorm_type, bias):
        ...
        # self.attention = Attention(self.dim // 2, num_heads, bias)
        # self.local_conv = nn.Conv2d(self.dim // 2, self.dim // 2, kernel_size=1, bias=bias)
        # self.local = GatedDegradationAdaption(self.dim // 2, initial_temperature=initial_temperature)
        self.local = Spatial_local_Enhanced(self.dim // 2, bias)
        self.attn = FreModule(self.dim // 2, num_heads, bias)
        # self.illumination_conv = nn.Conv2d(self.dim // 2, self.dim // 2, kernel_size=1, bias=bias)

        self.fuse_conv = nn.Conv2d(self.dim, self.dim // 2, kernel_size=1, bias=bias)
    ...
    def forward(self, x, y):
        ...
        # gate = x[:, 0::2, :, :]
        #
        # x_a = self.norms[0](attn)
        # x_a = self.attention(x_a)
        # x_l = self.norms[1](gate)
        # x_l = self.local(x_l)
        # x_i = self.norms[2](gate)
        # x_i = self.illumination(y, x_i)
        # fuse = self.fuse_conv(torch.cat([x_l, x_i], dim=1))
        # x = torch.cat([x_a, fuse], dim=1)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # x = x + self.mixer(self.norms[0](x),y) * self.beta
...
if __name__ == '__main__':
    model = MyModel(rank=2,
                    ...
                    stage_depth=[3, 2, 1], depth_type="constant",
                    rank_type="spread",
