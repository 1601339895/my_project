...
def main():
    ...
    StyleFilter_optimizer = torch.optim.Adamax([p for p in StyleFilter.parameters() if p.requires_grad == True],
                                               lr=args.lr_style)
    ...
    raindrop_loader_style = data.DataLoader(raindrop_dataset, batch_size=args.batch_size_style, shuffle=True,
    ...
    rain_loader_style = data.DataLoader(rain_dataset, batch_size=args.batch_size_style, shuffle=True,
                                        num_workers=args.num_workers,
                                        pin_memory=True, drop_last=True)
    ...
    snow_loader_style = data.DataLoader(snow_dataset, batch_size=args.batch_size_style, shuffle=True,
                                        num_workers=args.num_workers,
                                        pin_memory=True, drop_last=True)
    ...
    for i_iter in tqdm(range(0, args.num_steps)):
        ...
        try:  # raindrop
            ...
            raindrop_loader_style = data.DataLoader(raindrop_dataset, batch_size=args.batch_size_style, shuffle=True,
                                                    num_workers=args.num_workers,
                                                    pin_memory=True,
                                                    drop_last=True)  # load raindrop dataset (drop last)
            raindrop_loader_style_iter = enumerate(raindrop_loader_style)
            _, batch = raindrop_loader_style_iter.__next__()
        ...
        style_embeddings = torch.cat(
            (torch.unsqueeze(raindrop_vec[0], 0), torch.unsqueeze(rain_vec[0], 0), torch.unsqueeze(snow_vec[0], 0),
        ...
        if i_iter == args.num_steps - 1:  # save style discriminators in the end
            torch.save(StyleFilter.state_dict(),
                       osp.join(args.snapshot_dir, run_name) + '_stylefilter_' + str(i_iter) + '.pth')
