...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        x = self.project_in(x)  # 扩张维度，输出维度为4倍的输入特征图维度
...
class Mix(nn.Module):
    def __init__(self, dim, num_heads, initial_temperature, LayerNorm_type, bias):
        ...
        self.a_conv = nn.Conv2d(dim, dim // 2, 1)
        self.l_conv = nn.Conv2d(dim, dim // 4, 1)
        self.i_conv = nn.Conv2d(dim, dim // 4, 1)
        self.attention = Attention(self.dim, num_heads, bias)
        self.local = GatedDegradationAdaption(self.dim // 2, initial_temperature=initial_temperature)
        self.illumination = Illu_Fre(self.dim // 2, bias)
        ...
        self.init_conv = nn.Sequential(
            nn.Conv2d(dim, 2 * (dim // 2 + 2 * (dim // 4)), 1),
        )
        self.finall_conv = nn.Conv2d(2 * (dim // 2 + 2 * (dim // 4)), dim, 1)

    def forward(self, x):
        # x: (B, C, H, W)
        B, C, H, W = x.shape  # B=1, C=64, H=256, W=256
        x = self.init_conv(x)
        x_a = x[:, :self.dim]
        x_l = x[:, self.dim:self.dim + self.dim // 2]
        x_i = x[:, self.dim + self.dim // 2:]

        x_a = self.attention(self.norms[0](x_a))
        x_l = self.local(self.norms[1](x_l))
        x_i = self.illumination(self.norms[2](x_i))
        x = torch.cat([x_a, x_l, x_i], dim=1)
        x = self.gelu(x)  # [B,C,H,W]
        x_ca = self.ca(x) * x
        out = self.ca_conv(x_ca)
        return out  # (B, C, H, W)
...
class MyModel(nn.Module):
    def __init__(self,
                 ...
                 LayerNorm_type='HolisticWithBias',
                 bias=False,
                 rank=2,
                 num_experts=4,
                 depth_type='lin',
