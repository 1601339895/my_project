...
class Illu_Fre(nn.Module):

    def __init__(self, dim, bias):
        super(Illu_Fre, self).__init__()
        hidden_channels = dim * 2
        self.process1 = nn.Sequential(
            nn.Conv2d(dim, hidden_channels, kernel_size=1, bias=bias),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Conv2d(hidden_channels, dim, kernel_size=1, bias=bias))
...
class GatedDegradationAdaption(nn.Module):
    def __init__(self, dim, initial_temperature=1.0):
        super(GatedDegradationAdaption, self).__init__()
        hidden_channels = dim * 2

        self.expansion = nn.Conv2d(dim, hidden_channels, kernel_size=1)
        self.depthwise_conv = nn.Conv2d(hidden_channels // 4, hidden_channels // 2, kernel_size=3, padding=1,
                                        groups=hidden_channels // 4)
        self.gate_combination = nn.Conv2d(hidden_channels // 4 + hidden_channels // 2, hidden_channels // 2,
                                          kernel_size=1)
    ...
    def forward(self, F_in):
        # F_in: (B, C, H, W)
        mu = torch.mean(F_in, dim=[2, 3], keepdim=True)
        sigma = torch.sqrt(torch.mean((F_in - mu) ** 2, dim=[2, 3], keepdim=True))
        delta = torch.sigmoid(mu + sigma)
        tau_adj = self.temperature * delta
        F_prime = self.expansion(F_in)

        total_channels = F_prime.size(1)
        alpha_channels = total_channels // 4
        beta_channels = total_channels // 4
        gamma_channels = total_channels - beta_channels - alpha_channels
        gamma, beta, alpha = torch.split(
            F_prime,
            [gamma_channels, beta_channels, alpha_channels],
            dim=1
        )
        alpha_prime = self.depthwise_conv(alpha) * (1 + tau_adj)
        gamma = self.activation(gamma)
        concatenated_features = torch.cat([beta, alpha_prime], dim=1)
        F_gated = self.gate_combination(concatenated_features)
        F_gated = gamma * F_gated
        F_out = F_gated + F_in
...
class MSFN(nn.Module):
    ...
    def __init__(self, dim, ffn_expansion_factor, bias):
        ...
        self.conv_init = nn.Sequential(  # PW->DW->
            nn.Conv2d(dim, hidden_channels, 1),
        )
        ...
        self.conv_fina = nn.Sequential(
            nn.Conv2d(hidden_channels, dim, 1),
        )
...
class Mlp(nn.Module):
    def __init__(self, dim, bias):
        ...
        self.act = nn.GELU()
        self.dwconv = nn.Conv2d(dim, dim, 5, 1, 5 // 2, groups=dim, bias=bias)
...
class AdaptIR(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias):
        ...
        self.hidden = dim // ffn_expansion_factor
...
class Mix(nn.Module):
    def __init__(self, dim, num_heads, initial_temperature, LayerNorm_type, bias):
        super(Mix, self).__init__()
        self.dim = dim

        # 分支归一化层
        self.norm_attn = LayerNorm(dim, LayerNorm_type)
        self.norm_local = LayerNorm(dim // 2, LayerNorm_type)
        self.norm_illum = LayerNorm(dim // 2, LayerNorm_type)

        # 分支主干
        self.attention = Attention(dim, num_heads, bias)
        self.local = GatedDegradationAdaption(dim // 2, initial_temperature=initial_temperature)
        self.illumination = Illu_Fre(dim // 2, bias)

        # 投影到统一通道维度
        self.proj_local = nn.Conv2d(dim // 2, dim, kernel_size=1)
        self.proj_illum = nn.Conv2d(dim // 2, dim, kernel_size=1)
        ...
        self.final_proj = nn.Conv2d(dim * 3, dim, kernel_size=1)
        self.residual_proj = nn.Conv2d(dim, dim, kernel_size=1)
        self.gelu = nn.GELU()
    ...
    def forward(self, x):
        ...
        x_local = x[:, 0::2, :, :]
        # 分支处理
        x_attn = self.attention(self.norm_attn(x_attn))
        x_loc = self.local(self.norm_local(x_local))
        x_ill = self.illumination(self.norm_illum(x_local))

        # 统一通道维度
        x_attn = x_attn
        x_loc = self.proj_local(x_loc)
        x_ill = self.proj_illum(x_ill)

        # 多分支拼接
        x_fused = torch.cat([x_attn, x_loc, x_ill], dim=1)
        x_fused = self.final_proj(x_fused)
        x_fused = self.gelu(x_fused)

        # 通道注意力加权
        ca_weight = self.ca(x_fused)
        x_out = ca_weight * x_fused
...
class EncoderBlock(nn.Module):
    def __init__(self, dim, num_heads, initial_temperature, ffn_expansion_factor, LayerNorm_type, bias):
        super().__init__()

        drop_path = 0.0
        self.norms = nn.ModuleList([
            LayerNorm(dim, LayerNorm_type),  # dim=64
            LayerNorm(dim, LayerNorm_type)  # dim=64
        ])

        self.mixer = Mix(dim, num_heads, initial_temperature, LayerNorm_type, bias)
...
class EncoderResidualGroup(nn.Module):
    def __init__(self,
                 dim: int, num_heads: List[int], num_blocks: int, initial_temperature: float, ffn_expansion_factor: int,
        ...
        for i in range(num_blocks):
            self.layers.append(
                EncoderBlock(dim=dim, num_heads=num_heads, initial_temperature=initial_temperature,
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, freq_emb=None):
        ...
        x = self.mixer(x_a, x_s) * self.beta + shortcut
...
class MyModel(nn.Module):
    def __init__(self,
        ...
        # -- Patch Embedding
        self.patch_embed = OverlapPatchEmbed(in_c=in_c, embed_dim=dim, bias=False)
        ...
        #     ffn_expansion_factor=ffn_expansion_factor,
        #     LayerNorm_type=LayerNorm_type, bias=True, )
        self.output = nn.Conv2d(dim, out_c, kernel_size=3, stride=1, padding=1, bias=bias)
...
if __name__ == '__main__':
    model = MyModel(rank=2,
                    ...
                    stage_depth=[1, 1, 1], depth_type="constant",
                    rank_type="spread",
